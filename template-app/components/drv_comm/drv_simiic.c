...
{
    uint16_t j = 20; //使用小钻风摄像头时，此延时需要设置为500左右  //ICM等传感器应设置为20
    while (j--)
        ;
}
...
{
    GpioInit(SEEKFREE_SCL, GPIO_MODE_OUTPUT_OD);
    gpio_set_pull_mode(SEEKFREE_SCL, GPIO_PULLUP_ONLY);
    GpioInit(SEEKFREE_SDA, GPIO_MODE_OUTPUT_OD);
    gpio_set_pull_mode(SEEKFREE_SDA, GPIO_PULLUP_ONLY);
}